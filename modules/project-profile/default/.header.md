...
When `using_admin_project = true`, the provisioning role ARN is left null on each blueprint configuration so the admin project's execution role acts as the provisioner.
